...
interface QuoteTemplateManagerModalProps {
  ...
  onResyncExposureAttributes: (id: string) => void;
  categories: string[];
            ...
            <ul className="space-y-2">
              {registrations.map(r => (
                <RegistrationListItem
                  ...
                  onResyncExposureAttributes={onResyncExposureAttributes}
                />
              ))}
            </ul>
          )}
...
const RegistrationListItem: React.FC<RegistrationListItemProps> = ({ registration, onDelete, onUpdateCustomFieldNames, onUpdateOptionFieldName, onResyncExposureAttributes }) => {
    ...
    <li className="bg-slate-700/50 border border-slate-600 rounded-md px-3 py-2">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button
            onClick={() => onResyncExposureAttributes(registration.id)}
            ...
            title="이 견적서의 노출속성 항목을 기본 항목(색상, 수량)과 다시 비교해서, 기본 항목에는 없는 항목(예: 높이)을 '추가 항목 이름'에 채워 넣습니다."
          >
            노출속성 다시 비교
          </button>
          <button
