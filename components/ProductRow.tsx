...
import { TrashIcon, ImageIcon, CopyIcon, UploadIcon, MemoIcon, CalculatorIcon, CopyFromAboveIcon, DownloadIcon, SpinnerIcon, SparklesIcon, ChevronDownIcon, DocumentAddIcon, ImageEditIcon, CloseIcon, ClipboardIcon, SaveIcon } from './Icons';
...
const Field: React.FC<{ label: string; className?: string; children: React.ReactNode }> = ({ label, className = '', children }) => (
  <div className={`flex flex-col gap-1 min-w-0 ${className}`}>
    <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider px-0.5 whitespace-nowrap overflow-hidden text-ellipsis" title={label}>{label}</label>
    {children}
  </div>
);
...
const ProductRow: React.FC<ProductRowProps> = ({
  ...
  const inputClass = "w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200";
  const numericInputClass = `${inputClass} text-right`;
  const fileInputClass = "w-full px-2 py-1 text-xs bg-slate-700 border border-slate-600 rounded-md text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition duration-200";

  const formatNumber = (value: string | number) => Number(value || 0).toLocaleString();

  return (
    <div className="bg-slate-800 rounded-xl border border-slate-700/60 shadow-lg hover:border-slate-600/80 transition-colors duration-150">
      {/* Top bar: index, quick actions, thumbnail, category, management */}
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-700/60">
        <span className="flex-shrink-0 w-6 text-center text-sm font-semibold text-slate-500">{displayIndex + 1}</span>

        <div className="flex items-center gap-0.5 flex-shrink-0">
          <label
            className={`relative flex flex-col items-center justify-center gap-0.5 text-teal-400 hover:text-teal-300 transition-colors duration-200 px-1.5 py-1 rounded-md hover:bg-teal-500/10 cursor-pointer ${isImportingQuote ? 'opacity-50 pointer-events-none' : ''}`}
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-0.5 transition-colors duration-200 px-1.5 py-1 rounded-md hover:bg-yellow-500/20 ${product.memo ? 'text-yellow-400 hover:text-yellow-300' : 'text-slate-400 hover:text-slate-200'}`}
          ...
          <button
            ...
            className="text-green-500 hover:text-green-400 transition-colors duration-200 p-1.5 rounded-md hover:bg-green-500/10"
          ...
          {displayIndex > 0 && (
            <button
              ...
              className="text-sky-500 hover:text-sky-400 transition-colors duration-200 p-1.5 rounded-md hover:bg-sky-500/10"
        ...
        <div className="relative flex-1 min-w-[160px]" ref={categoryMenuRef}>
          <div className="relative flex items-center">
            ...
            <button
              ...
              className="absolute right-1 text-slate-400 hover:text-blue-400 p-1 transition-colors"
              title="카테고리 프리셋"
            >
              <ChevronDownIcon />
            </button>
          ...
          {showCategoryMenu && (
            <div className="absolute left-0 top-full mt-2 w-72 max-h-96 overflow-y-auto bg-slate-800 border border-slate-600 rounded-lg shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] z-[9999] py-2 animate-in fade-in slide-in-from-top-2 duration-200">
              {registeredCategories.length > 0 && (
                <>
                  <div className="px-3 py-1.5 border-b border-slate-700 bg-slate-700/30 mb-1">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">등록된 카테고리</span>
                  </div>
                  {registeredCategories.map(c => (
                    <button
                      key={c}
                      onClick={() => handleSelectPreset(c)}
                      className="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:bg-blue-600 hover:text-white transition-colors border-b border-slate-700/50 last:border-0"
                    >
              ...
              <div className="px-3 py-1.5 border-b border-slate-700 bg-slate-700/30 mb-1">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">카테고리 빠른 선택</span>
              </div>
              {Object.entries(CATEGORY_PRESETS).map(([name, fullPath]) => (
                <button
                  key={name}
                  onClick={() => handleSelectPreset(fullPath)}
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:bg-blue-600 hover:text-white transition-colors border-b border-slate-700/50 last:border-0 group"
                >
                  <div className="font-semibold">{name}</div>
                  <div className="text-[10px] text-slate-500 mt-0.5 group-hover:text-blue-100 line-clamp-1">{fullPath}</div>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0 ml-auto">
          <button
            ...
            className="flex flex-col items-center justify-center gap-0.5 text-orange-400 hover:text-orange-300 transition-colors duration-200 px-1.5 py-1 rounded-md hover:bg-orange-500/10 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <button
            ...
            className="flex flex-col items-center justify-center gap-0.5 text-emerald-400 hover:text-emerald-300 transition-colors duration-200 px-1.5 py-1 rounded-md hover:bg-emerald-500/10"
          ...
          <label
            className="flex flex-col items-center justify-center gap-0.5 px-1 py-1 rounded-md cursor-pointer select-none text-slate-500 hover:text-slate-300 hover:bg-slate-500/10 transition-colors duration-200"
          ...
          <button
            ...
            className="flex flex-col items-center justify-center gap-0.5 text-purple-400 hover:text-purple-300 transition-colors duration-200 px-1.5 py-1 rounded-md hover:bg-purple-500/10 disabled:opacity-30 disabled:cursor-not-allowed"
      ...
      <div className="flex flex-wrap gap-3 px-4 pt-3">
        <Field label="URL" className="flex-[2] basis-[220px] relative">
          <div className="relative flex items-center">
            <input
              type="text"
              name="url"
            ...
            <button
              ...
              disabled={isDownloading || !product.url}
              className="absolute right-1 text-slate-400 hover:text-blue-400 p-1 rounded-full hover:bg-blue-500/10 transition-colors duration-200 disabled:opacity-30"
              title="이미지 다운로드 시도"
            >
              {isDownloading ? <SpinnerIcon className="w-4 h-4 animate-spin text-blue-400" /> : <DownloadIcon />}
            </button>
          </div>
        </Field>
        ...
        <Field label="원가" className="flex-1 basis-[120px] relative group">
          <div className="relative flex items-center">
            ...
            <button
              ...
              className="absolute right-1 text-slate-400 hover:text-blue-400 p-1 rounded-full hover:bg-blue-500/10 transition-colors duration-200"
              aria-label="Open Margin Calculator"
            >
              <CalculatorIcon />
            </button>
          </div>
          <div className="absolute top-1/2 -translate-y-1/2 left-full ml-2 w-max p-3 bg-slate-900 border border-slate-600 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
            <div className="flex items-center space-x-4 text-sm whitespace-nowrap">
              <div className="flex items-center gap-1">
                <span className="font-semibold text-slate-400">공급가:</span>
                <span className="text-emerald-400 font-mono">{formatNumber(product.supplyPrice)}원</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="font-semibold text-slate-400">판매가:</span>
                <span className="text-emerald-400 font-mono">{formatNumber(product.sellingPrice)}원</span>
              </div>
              <div className="flex items-center gap-1 border-l border-slate-700 pl-3">
                <span className="font-semibold text-slate-400">마진:</span>
                <span className="text-green-400 font-mono">{formatNumber(product.margin)}원</span>
              </div>
            </div>
            <div className="absolute top-1/2 -translate-y-1/2 left-[-6px] w-0 h-0 border-t-[6px] border-t-transparent border-b-[6px] border-b-transparent border-r-[6px] border-r-slate-600"></div>
          </div>
        </Field>

        <Field label="검색어" className="flex-1 basis-[130px]">
          <input type="text" name="searchKeyword" value={product.searchKeyword} onChange={handleChange} onFocus={handleFocus} className={inputClass} />
        </Field>
      ...
      <div className="flex flex-wrap gap-3 px-4 pb-4 pt-3">
        ...
        <Field label="한 개 단품 포장 사이즈" className="flex-[1.2] basis-[160px]">
          <div className="flex items-center gap-1.5">
            ...
            />
            <label className="flex items-center gap-1 flex-shrink-0 text-[11px] text-slate-400 whitespace-nowrap cursor-pointer" title="상품 사이즈(가로/세로/높이)와 동일한 값을 자동으로 사용">
              <input
                type="checkbox"
        ...
        <Field label="중량(g)" className="basis-[75px]">
          <div className="relative">
            ...
            <span className="absolute inset-y-0 right-1.5 flex items-center text-slate-400 text-xs">g</span>
          </div>
        </Field>

        <Field label="색상" className="basis-[85px]">
        ...
        <Field label="SKU" className="basis-[85px]">
        ...
        <Field label="대표 이미지" className="basis-[104px]">
          <div className="relative flex items-center">
            <label htmlFor={`thumbnail-file-upload-${product.id}`} className="absolute left-1 z-10 text-slate-400 hover:text-blue-400 cursor-pointer p-1 rounded-full hover:bg-blue-500/20 transition-colors">
              <UploadIcon />
            </label>
            ...
            {product.thumbnailDataUrl && (
              <button
                ...
                className="absolute right-1 text-purple-400 hover:text-purple-300 transition-colors"
                title="이미지 내 텍스트 번역"
              >
        ...
        <Field label="상세 이미지" className="basis-[104px]">
          <div className="relative flex items-center">
            <label htmlFor={`detail-file-upload-${product.id}`} className="absolute left-1 z-10 text-slate-400 hover:text-blue-400 cursor-pointer p-1 rounded-full hover:bg-blue-500/20 transition-colors">
              <UploadIcon />
            </label>
        ...
        <Field label="제품 필수 표시사항" className="basis-[104px]">
          <div className="relative flex items-center">
            <label htmlFor={`label-file-upload-${product.id}`} className="absolute left-1 z-10 text-slate-400 hover:text-blue-400 cursor-pointer p-1 rounded-full hover:bg-blue-500/20 transition-colors">
              <UploadIcon />
            </label>
        ...
        <Field label="이미지 ZIP" className="basis-[44px]">
          <button
            ...
            className="w-full flex items-center justify-center px-2 py-1.5 bg-slate-700 border border-slate-600 rounded-md text-emerald-300 hover:bg-slate-600 hover:text-emerald-200 transition-colors disabled:opacity-30 disabled:cursor-not-allowed [&_svg]:h-3.5 [&_svg]:w-3.5 [&_svg]:mr-0"
        ...
        <Field label="통합다운" className="basis-[44px]">
          <button
            onClick={() => onIntegratedDownload(product.id)}
            disabled={isIntegratedDownloading}
            className="w-full flex items-center justify-center px-2 py-1.5 bg-slate-700 border border-slate-600 rounded-md text-amber-300 hover:bg-slate-600 hover:text-amber-200 transition-colors disabled:opacity-30 disabled:cursor-not-allowed [&_svg]:h-3.5 [&_svg]:w-3.5 [&_svg]:mr-0"
            title="상품명 폴더에 라벨(자동생성) + 대표/상세 이미지(zip) + 견적서를 한번에 다운로드 (준비된 항목만 저장)"
          >
            {isIntegratedDownloading ? <SpinnerIcon className="animate-spin" /> : <SaveIcon />}
          </button>
        </Field>
        ...
        <Field label="라벨" className="basis-[44px]">
          <button
            ...
            className="w-full flex items-center justify-center px-2 py-1.5 bg-slate-700 border border-slate-600 rounded-md text-blue-300 hover:bg-slate-600 hover:text-blue-200 transition-colors [&_svg]:h-3.5 [&_svg]:w-3.5"
            title="라벨 생성"
          >
            <ImageIcon />
          </button>
          ...
          <Field key={name} label={name} className="basis-[100px]">
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 text-slate-400 hover:text-red-400 transition-colors p-0.5 rounded-full hover:bg-red-500/10 [&_svg]:h-3.5 [&_svg]:w-3.5"
        ...
        <Field label="이미지 편집" className="basis-[44px]">
          <button
            ...
            className="w-full flex items-center justify-center px-2 py-1.5 bg-slate-700 border border-slate-600 rounded-md text-purple-300 hover:bg-slate-600 hover:text-purple-200 transition-colors [&_svg]:h-3.5 [&_svg]:w-3.5"
        ...
        <Field label="상세페이지" className="basis-[44px]">
          <button
